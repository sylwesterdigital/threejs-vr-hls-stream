...
function loadModel(modelFile,modelName,position,scale,animPosition,speed,rotation) {
...
loader.setDRACOLoader( dracoLoader );  
  ...
  loader.load(modelFile,
    ...
    function (gltf) {
      ...
      w.mixers[modelName] = new THREE.AnimationMixer(ref);
...
function loadModels() {
  ...
  loadModel('./assets/models/gltf/box.glb','box',{x:0,y:0,z:0},{x:1,y:1,z:1},0,{speed:2},{x:0,y:0,z:0})    
...
function loadTV(videoSrc) {
  ...
  if (Hls.isSupported()) {
    ...
    hls.on(Hls.Events.MANIFEST_PARSED, function() {
      ...
      console.log(' == hls.on(Hls.Events.MANIFEST_PARSED')
      ...
      if (playPromise !== undefined) {
        ...
        .catch(error => {
          console.log('playPromise.then - error')
    ...
    video.addEventListener('loadedmetadata', function() {
      ...
      console.log(' == video.addEventListener loadedmetadata ')
      //video.play();
      ...
      if (playPromise !== undefined) {
        playPromise.then(_ => {
          console.log('playPromise.then - loadedmetadata')
          // Automatic playback started!
          // Show playing UI.
        })
        .catch(error => {
          console.log('playPromise error - loadedmetadata')
          // Auto-play was prevented
          // Show paused UI.
        });
      }
...
function init() {
  ...
  const light3 = new THREE.DirectionalLight(0x00ff00);
